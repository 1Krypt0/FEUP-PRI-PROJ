...
fill_missing:
	python3 milestone-1/fill_missing.py

clean_data:
	python3 milestone-1/clean_data.py

separate_authors:
	python3 milestone-1/separate_authors.py

extract_extra:
	python3 milestone-1/extract_tags_sections.py

# Convert with pretier so that it is properly formatted
convert_formats:
	python3 milestone-1/convert_formats.py && npx prettier --write data/spacenews.json
